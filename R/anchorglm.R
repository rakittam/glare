...
#' @param A_formula an object of class \code{"\link{formula}"} for the anchor variables.
...
anchorglm <- function(formula, A_formula, data, xi, m = 1,
  ...
  anchor_objective <- function(b.hat) {
    1/n.obs * (-log_likelihood(b = b.hat, Y = Y, X = X, linkinv = linkinv, m = m) +
               xi * anchor_penalty(R = anch_res(b = b.hat, Y = Y, X = X, linkinv = linkinv, m = m, family = family), A = A))
  }
